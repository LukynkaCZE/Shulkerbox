...
class PropCommands {
    ...
    init {
        ...
        cm.command(
            propAxisCommandBase.literal("flip")
        ...
        cm.command(
            propCommandBase.literal("create")
                .handler { ctx ->
        ...
        cm.command(
            propCommandBase.literal("brightness")
        ...
        cm.command(
            propCommandBase.literal("shift_all")
                .required("x", floatParser())
                .required("y", floatParser())
                .required("z", floatParser())
                .handler { ctx ->
        ...
        cm.command(
            propCommandBase.literal("remove")
                .handler { ctx ->
                    val player = ctx.sender() as Player
                    val map = MapManager.selectedShulkerboxMap(player)
                    val prop = PropManager.propSelections[player]
        ...
        cm.command(
            propCommandBase.literal("unselect")
                .handler { ctx ->
                    val player = ctx.sender() as Player
                    val prop = PropManager.propSelections[player]
                    if (prop == null) {
        ...
        cm.command(
            propCommandBase.literal("select")
                .handler { ctx ->
                    val player = ctx.sender() as Player
                    val map = MapManager.selectedShulkerboxMap(player)
                    if (map == null) {
        ...
        cm.command(
            propCommandBase.literal("snap_rotation")
        ...
        cm.command(
            propCommandBase.literal("snap_location")
                .handler { ctx ->
                    val player = ctx.sender() as Player
                    val prop = PropManager.propSelections[player]
                    if (prop == null) {
                    ...
                    val location = prop.location.toBukkitVector().toLocation(player.world).toBlockLocation().add(0.5, 0.0, 0.5)
                    prop.location = location.toCenterLocation().toVector().toShulkerboxVector()
                    activeMap.updateDrawables()
                    player.playEditSound()
                })

        cm.command(
            propCommandBase.literal("item")
                .handler { ctx ->
                    val player = ctx.sender() as Player
                    val prop = PropManager.propSelections[player]
        ...
        if (ShulkerboxPaper.youkaiIntegration) {
            cm.command(
                propCommandBase.literal("youkai")
        ...
        cm.command(
            propCommandBase.literal("copy_from_map")
